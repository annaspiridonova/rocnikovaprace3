...
import cz.gyarab3e.rocnikovaprace3.services.NotFoundException;
...
// handling move, validation and unauthorized exceptions
@ControllerAdvice
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public final ResponseEntity<String> handleAllNotFoundExceptions(NotFoundException ex, WebRequest request) {
        String error = new String(ex.getNoGameError().name());
        return new ResponseEntity<>(error, HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler(AccessDeniedExceptions.class)
    public final ResponseEntity<String> handleAllAccessDeniedExceptions(NotFoundException ex, WebRequest request) {
